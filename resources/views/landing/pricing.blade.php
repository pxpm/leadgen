...
$plans = \App\Models\Plan::public()->orderBy('sort_order')->get();
@endphp
...
<section x-data="{ billing: 'yearly' }" class="relative pt-16 pb-28 px-6 bg-gray-50/70 overflow-hidden">
    ...
    <div class="relative max-w-xs mx-auto flex items-center justify-center gap-3">
        ...
        <button
            ...
            class="px-4 py-2 text-sm font-semibold rounded-lg transition-colors">
            {{ $p['billed_yearly'] }}
            <span class="ml-1.5 px-1.5 py-0.5 text-[11px] font-bold bg-green-100 text-green-700 rounded-full">-{{ (int) round((1 - (int) $plans->first()?->yearly_price_per_month / (int) $plans->first()?->monthly_price) * 100) }}%</span>
        </button>
    </div>
    ...
    <div class="relative max-w-6xl mx-auto grid md:grid-cols-3 gap-8 items-start">
        @foreach ($plans as $plan)
            @php
                ...
                $yearlyPrice = number_format($plan->yearly_price_per_month, 0);
                $yearlyTotal = $plan->yearly_price_per_month * 12;
                $monthlyTotal = $plan->monthly_price * 12;
                $savingsPercent = (int) round((1 - $plan->yearly_price_per_month / $plan->monthly_price) * 100);
                $smsLimit = $plan->getLimit('sms_monthly');
                $emailLimit = $plan->getLimit('email_monthly');
                $ingestionLimit = $plan->getLimit('email_ingestion_monthly');
                $hasRecoveryCall = (bool) ($plan->limits['recovery_call'] ?? false);
                $locale = app()->getLocale();
                $currencyBefore = $locale === 'en';
            @endphp

            <div class="card-lift relative bg-white border-2 {{ $isPopular ? 'border-amber-300 shadow-xl shadow-amber-100/50' : 'border-gray-200' }} rounded-3xl overflow-hidden {{ $isPopular ? 'md:-mt-6' : '' }}">
                @if ($isPopular)
                ...
                <div class="h-1.5 {{ $isPopular ? 'bg-amber-500' : 'bg-gray-300' }}"></div>
                <div class="p-8 sm:p-10 text-center">
                    <h2 class="text-lg font-bold tracking-widest uppercase {{ $isPopular ? 'text-amber-600' : 'text-gray-500' }} mb-2">
                        {{ $plan->name }}
                    </h2>
                    ...
                    <div x-show="billing === 'yearly'" class="mt-4">
                        <div class="flex items-baseline justify-center gap-0.5">
                            @if ($currencyBefore)<span class="text-xl text-gray-400 font-medium self-start mt-1.5">€</span>@endif
                            <span class="text-4xl sm:text-5xl font-extrabold text-gray-900">{{ $yearlyPrice }}</span>
                            @if (! $currencyBefore)<span class="text-xl text-gray-400 font-medium self-end mb-1">€</span>@endif
                            <span class="text-lg text-gray-400 font-medium">{{ $p['per_month'] }}</span>
                        </div>
                        <p class="mt-1.5 text-xs text-green-600 font-semibold">
                            {{ number_format($yearlyTotal, 0) }}€/ano
                            <span class="ml-1.5 px-1.5 py-0.5 bg-green-50 text-green-700 rounded-full">{{ str_replace('{percent}', (string) $savingsPercent, $p['save_label']) }}</span>
                        </p>
                    </div>
                    ...
                    <div x-show="billing === 'monthly'" class="mt-4">
                        <div class="flex items-baseline justify-center gap-0.5">
                            @if ($currencyBefore)<span class="text-xl text-gray-400 font-medium self-start mt-1.5">€</span>@endif
                            <span class="text-4xl sm:text-5xl font-extrabold text-gray-900">{{ $monthlyPrice }}</span>
                            @if (! $currencyBefore)<span class="text-xl text-gray-400 font-medium self-end mb-1">€</span>@endif
                            <span class="text-lg text-gray-400 font-medium">{{ $p['per_month'] }}</span>
                        </div>
                        <p class="mt-1.5 text-xs text-gray-400">
                            {{ number_format($monthlyTotal, 0) }}€/ano
                        </p>
                    </div>

                    <p class="mt-3 text-gray-500 text-sm leading-relaxed">{{ $plan->description }}</p>

                    <a href="#demo" class="mt-8 inline-flex items-center justify-center w-full px-6 py-3.5 text-sm font-bold text-white {{ $isPopular ? 'bg-amber-500 hover:bg-amber-600 shadow-md shadow-amber-200/50' : 'bg-gray-800 hover:bg-gray-900' }} rounded-xl transition-colors">
                        {{ $p['cta_button'] }}
                    </a>
                </div>
                ...
                <div class="border-t border-gray-100 px-8 sm:px-10 py-8">
                    ...
                    <ul class="space-y-4">
                        <li class="flex items-start gap-3">
                            <div class="shrink-0 w-9 h-9 rounded-lg {{ $isPopular ? 'bg-amber-50' : 'bg-gray-50' }} flex items-center justify-center">
                                <svg class="w-4.5 h-4.5 {{ $isPopular ? 'text-amber-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ number_format($smsLimit, 0, ',', '.') }} {{ $p['limit_sms'] }}</p>
                                <p class="text-xs text-gray-400 mt-0.5">{{ $p['limit_sms_desc'] }}</p>
                            </div>
                        </li>
                        <li class="flex items-start gap-3">
                            <div class="shrink-0 w-9 h-9 rounded-lg {{ $isPopular ? 'bg-amber-50' : 'bg-gray-50' }} flex items-center justify-center">
                                <svg class="w-4.5 h-4.5 {{ $isPopular ? 'text-amber-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ number_format($emailLimit, 0, ',', '.') }} {{ $p['limit_email'] }}</p>
                                <p class="text-xs text-gray-400 mt-0.5">{{ $p['limit_email_desc'] }}</p>
                            </div>
                        </li>
                        <li class="flex items-start gap-3">
                            <div class="shrink-0 w-9 h-9 rounded-lg {{ $isPopular ? 'bg-amber-50' : 'bg-gray-50' }} flex items-center justify-center">
                                <svg class="w-4.5 h-4.5 {{ $isPopular ? 'text-amber-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/></svg>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ number_format($ingestionLimit, 0, ',', '.') }} {{ $p['limit_ingestion'] }}</p>
